...
export const logoutRequest = async ({ userId, accessToken }: { userId: string; accessToken: string }) => {
  try {
...
export const getProfileRequest = async ({ accessToken }: { accessToken: string }) => {
  try {
    console.log('getProfileRequest input', { accessToken })

    const request = await axios.post(
      `${EXAM_API_URL}/GET_PROFILE`,
      {},
      {
        headers: {
          _token: accessToken,
